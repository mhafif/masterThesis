...
def buildDiscriminator(dis_flt, dis_kern, input_shape, activation, batchnorm, drop, drop_rate):
    ...
    cl_2 = Conv2D(dis_flt[1], dis_kern[1], strides=1, padding='same', kernel_initializer=normalInit(0.02))(cl_1)
    cl_3 = batchnorm_activation_dropout(cl_2, batchNorm=batchnorm, activation=activation, dropout=drop, dropRate=drop_rate)
    cl_4 = Conv2D(dis_flt[2], dis_kern[2], strides=1, padding='same', kernel_initializer=normalInit(0.02))(cl_3)
...
def buildGenerator(gen_flt, gen_kern, z_dim, size, gen_start_size, activation, batchnorm, drop, drop_rate):
    ...
    x = Dense(np.prod((size//4, size//4, gen_start_size)), kernel_initializer=normalInit(0.02))(input_generator)
    x = BatchNormalization()(x)
    x = Reshape((size//4, size//4, gen_start_size))(x)
    ...
    dcl_3 = batchnorm_activation_dropout(dcl_2, batchNorm=batchnorm, activation=activation, dropout=drop, dropRate=drop_rate)
    dcl_4 = Conv2DTranspose(gen_flt[2], gen_kern[2], strides=1, padding='same', kernel_initializer=normalInit(0.02))(dcl_3)
    # Output layer
...
def fitting_process(gan, gen, dis, input_train, epoch, latent_dim, noise_pct, label_smooth_r, label_smooth_f, run_folder):
    ...
    plotHistory(gen_loss_hist, dis_loss_hist, "Generator", "Discriminator", "GAN losses on MNIST", "loss", "epochs", filename)
    ...
    plotHistoryThree(gen_loss_hist, dis_loss_real_hist, dis_loss_fake_hist, "Generator", "Discriminator real", "Discriminator fake", 
                    "GAN losses (Real vs Fake) on MNIST", "loss", "epochs", filename)
...
def sampling_process(dis, gen, gan, run_folder, latent_dim, nb_samples, w_filename, datatype):
    ...
    for i in range(len(images)):
        filename = os.path.join(run_folder, "..", "..", "Evaluation", "mnistGAN", "sample{}.jpg".format(i))
...
if __name__ == "__main__":
    ...
    # Load MNIST dataset
    (input_train, _), (_, _) = mnist.load_data()
    ...
    gen_filters = [128, 64, NUM_CHANNELS]
    gen_kernels = [3, 3, 3]
    ...
    if TASK == "sampling":
        nb_realsamples = 200
        sampling_process(discriminator_model, generator_model, gan_model, CURRENT_DIR_RUN, LATENT_DIM, nb_realsamples, weights_filename, "mnist")
