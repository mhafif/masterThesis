...
def buildModels(enc_flt, dec_flt, enc_ker, dec_ker, z_dim, input_shape, activation, batchnorm, drop, drop_rate):
    ...
    decoder_output   = Conv2DTranspose(filters=dec_flt[3], kernel_size=dec_ker[3], activation='sigmoid', padding='same', name='decoder_output')(cx)
    # Instantiate decoder
...
def fitting_process(encoder, decoder, vae, input_train, input_test, epoch, latent_dim, run_folder):
    ...
    plotHistory(train_loss_hist, validation_loss_hist, "train set", "validation set", "VAE loss on CELEBA", "loss", "epochs", filename)
    filename = os.path.join(run_folder,"vae_accuracy.png")
    plotHistoryAcc(train_acc_hist, validation_acc_hist, "train set", "validation set", "VAE accuracy on CELEBA", "accuracy", "epochs", filename)
...
def sampling_process(encoder, decoder, vae, run_folder, latent_dim, nb_samples, w_filename, datatype):
    ...
    for i in range(len(images)):
        filename = os.path.join(run_folder, "..", "..", "Evaluation", "celebaVAE", "sample{}.jpg".format(i))
...
if __name__ == "__main__":
    ...
    input_test = celeba_dataset[train_test_split:train_test_split+5600,:]
    ...
    if TASK == "sampling":
        nb_realsamples = 200
        sampling_process(encoder, decoder, vae, CURRENT_DIR_RUN, LATENT_DIM, nb_realsamples, weights_filename, "celeba")
