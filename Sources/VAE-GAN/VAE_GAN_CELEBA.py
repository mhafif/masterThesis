...
def build_models(enc_flt, dec_flt, dis_flt, enc_ker, dec_ker, dis_ker, activation, batchnorm, drop, drop_rate):
    ...
    cx      = batchnorm_activation_dropout(cx, batchNorm=batchnorm, activation="relu", dropout=drop, dropRate=drop_rate)
    cx      = Conv2D(filters=enc_flt[2], kernel_size=enc_ker[2], strides=2, padding='same')(cx)
    cx      = batchnorm_activation_dropout(cx, batchNorm=batchnorm, activation="relu", dropout=drop, dropRate=drop_rate)
    x       = Flatten()(cx)
    x       = Dense(64)(x)
    ...
    cx  = batchnorm_activation_dropout(cx, batchNorm=batchnorm, activation="relu", dropout=drop, dropRate=drop_rate)
    cx  = Conv2DTranspose(filters=dec_flt[2], kernel_size=dec_ker[2], strides=2, padding='same')(cx)
    cx  = batchnorm_activation_dropout(cx, batchNorm=batchnorm, activation="relu", dropout=drop, dropRate=drop_rate)
    # decoder output layer
    decoded   = Conv2DTranspose(filters=dec_flt[3], kernel_size=dec_ker[3], activation='sigmoid', padding='same', name='decoder_output')(cx)
    # Instantiate decoder
    ...
    cl_3 = Conv2D(filters=dis_flt[2], kernel_size=dis_ker[2], strides=1, padding='same')(cl_2)
    cl_3 = batchnorm_activation_dropout(cl_3, batchNorm=batchnorm, activation=activation, dropout=drop, dropRate=drop_rate)
    cl_4 = Conv2D(filters=dis_flt[3], kernel_size=dis_ker[3], strides=1, padding='same')(cl_3)
...
def run_training(train_dataset, encoder, decoder, discriminator, vaegan, epoch_start, epochs, optimizers, gamma, noise, run_folder, label_smooth_r, label_smooth_f, history=None):
    if epoch_start != 0:
        enc_loss_hist, dec_loss_hist, dis_loss_hist = history[0], history[1], history[2]
    else:
        enc_loss_hist, dec_loss_hist, dis_loss_hist = np.array([]), np.array([]), np.array([])
    ...
    for epoch in range(epoch_start, epochs):
        ...
        if (epoch+1) % 1 == 0:
            ...
            with open(os.path.join(run_folder,"history"), "wb") as fp:
                history = np.array([enc_loss_hist, dec_loss_hist, dis_loss_hist])
...
def fitting_process(encoder, decoder, discriminator, vae, vaegan, input_train, epochs, latent_dim, 
                    run_folder, lr_enc, lr_dec, lr_dis, gamma, label_smooth_r, label_smooth_f, file_epoch_weight=None):
    history = None
    epoch_start = 0
    if file_epoch_weight != None:
        # Load weights
        vaegan.load_weights(file_epoch_weight)
        # fix epochs
        epoch_start = int(file_epoch_weight[file_epoch_weight.find('s-')+2:file_epoch_weight.find('.')])
        # load history save
        with open(os.path.join(run_folder,"history"), "rb") as fp:
    ...
    enc_loss_hist, dec_loss_hist, dis_loss_hist = run_training(train_dataset, encoder, decoder, discriminator, vaegan, epoch_start, epochs, 
                                                    [optimizer_enc, optimizer_dec, optimizer_dis], gamma, noise, run_folder, label_smooth_r, 
                                                    label_smooth_f, history=history)
    # Plotting the loss function of the gan model
    filename = os.path.join(run_folder,"VAEGAN_encdec_losses.png")
    plotHistory(enc_loss_hist, dec_loss_hist, "Encoder", "Decoder", "VAE-GAN Encoder-Decoder losses on CELEBA", "loss", "epochs", filename)
    filename = os.path.join(run_folder,"VAEGAN_disc_losses.png")
    plotHistoryOne(dis_loss_hist, "Discriminator", "VAE-GAN discriminator loss on CELEBA", "loss", "epochs", filename)
...
def sampling_process(encoder, decoder, vaegan, run_folder, latent_dim, nb_samples, w_filename, datatype):
    ...
    for i in range(len(images)):
        filename = os.path.join(run_folder, "..", "..", "Evaluation", "celebaVAEGAN", "sample{}.jpg".format(i))
...
if __name__ == "__main__":
    ...
    train_test_split = 22400  
    celeba_dataset = get_celeba_dataset(dataset_size, 51200, CELEBA_PATH)
    train_images = celeba_dataset[:train_test_split,:]
    test_images = celeba_dataset[train_test_split:train_test_split+5600,:]
    ...
    enc_kernels = [3, 3, 3]
    dec_filters = [256, 128, 64, NUM_CHANNELS]
    dec_kernels = [3, 3, 3, 3]
    ...
    if sys.argv[1] == "-train":
      ...
      print("Wrong arguments, choose between '-train <?weightFile>', '-sample <weightFile>' and '-plot <weightFile>'")

    if TASK == "training":
        fitting_process(encoder, decoder, discriminator, vae, vaegan, train_dataset, NO_EPOCHS, LATENT_DIM, 
                CURRENT_DIR_RUN, LR_ENC, LR_DEC, LR_DIS, GAMMA, REAL_LABEL_SMOOTHING, FAKE_LABEL_SMOOTHING, file_epoch_weight=weights_filename)
        
    if TASK == "sampling":
        nb_realsamples = 200
        sampling_process(encoder, decoder, vaegan, CURRENT_DIR_RUN, LATENT_DIM, nb_realsamples, weights_filename, "celeba")
